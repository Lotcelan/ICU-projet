...
def read_first_last_matrix(filename, read_meta_data=False):
    ...
    for i in range(nb):
        ...
        res.append(temp)
    if read_meta_data:
        return (res[0], res[1], rows, cols, nb_sub, T_e, Vitesse_air, Volume_air, L, l, K, D, offset_floor, offset_l_wall, offset_r_wall)
    else:
...
def enthalpy_calc(air_temp_last_first_file="./results/air_temp_last_first.tipe", masses_last_first_file="./results/masses_last_first.tipe"):
    ...
    return (h_tot, nb_sub, T_e, Vitesse_air, Volume_air, L, l, K, D, offset_floor, offset_l_wall, offset_r_wall)
